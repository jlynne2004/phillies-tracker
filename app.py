...
div[data-testid="stMetric"] { background:#ffffff; border:1px solid #e0e0e0; border-radius:8px; padding:12px; }
div[data-testid="stMetric"] label { color:#888 !important; font-size:11px !important; text-transform:uppercase; letter-spacing:1px; }
div[data-testid="stMetric"] div[data-testid="stMetricValue"] { color:#1a1a1a !important; font-size:22px !important; }
...
.stButton button { background:#ffffff !important; color:#999 !important; border:1px solid #ddd !important; border-radius:8px !important; }
.stButton button:hover { border-color:#c8102e !important; color:#c8102e !important; }
...
with tab_team:
        ...
        if recent:
            ...
            if num_games <= 1:
                ...
                for game_num in range(num_games):
                    ...
                    rest_ba = f"{rest_h/rest_ab:.3f}".lstrip("0") if rest_ab>0 else ".000"
                    team_ba = f"{t_h/t_ab:.3f}".lstrip("0") if t_ab>0 else ".000"
                    st.markdown(f'<div style="font-size:11px;color:#c8102e;margin-bottom:6px;margin-top:8px">Game {game_num+1}</div>', unsafe_allow_html=True)
                    gc1,gc2,gc3 = st.columns(3)
                    gc1.metric("Core 4 BA", c4_ba, f"{c4_h}H / {c4_ab}AB")
                    gc2.metric("Rest of Lineup", rest_ba, f"{rest_h}H / {rest_ab}AB")
                    gc3.metric("Full Team", team_ba, f"{t_h}H / {t_ab}AB")
    ...
    if hot_count >= 3:
        st.markdown(f'<div style="background:rgba(74,222,128,0.12);border:1px solid rgba(74,222,128,0.3);border-radius:10px;padding:12px 16px;text-align:center;font-size:14px;color:#4ade80;margin-bottom:16px">🔥 The $97.7M is ON FIRE! ({hot_count}/4 players on a hit streak)</div>', unsafe_allow_html=True)
    elif cold_count >= 3:
    ...
    for rank, player in enumerate(lb_data):
        ...
        st.markdown(f"""
        <div style="display:flex;align-items:center;padding:12px 8px;border-bottom:1px solid #eeeeee;gap:12px">
...
with tab_players:
    ...
    for player in PLAYERS:
        ...
        drought_bar = f'<div style="background:rgba(200,16,46,0.08);padding:8px 16px;border-top:1px solid rgba(200,16,46,0.2);text-align:center;font-size:12px;color:#c8102e">&#9888; {games_since}-game hitless streak &mdash; {ab_since} AB without a hit</div>' if drought else ""
        ...
        if has_splits:
            splits_html = (
                f'<div style="display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #eeeeee">'
                f'<div style="padding:10px 12px;border-right:1px solid #eeeeee">'
        ...
        html = (
            ...
            f'<div style="display:flex;align-items:center;padding:14px 16px;border-bottom:1px solid #eeeeee;gap:12px;position:relative">'
            f'{spark}'
            ...
            f'</div></div>'
            f'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;border-bottom:1px solid #eeeeee">{cell_last}{cell_elap}{cell_gab}</div>'
...
with tab_log:
    if log_df.empty:
        ...
        def highlight_multihit(row):
            if row.get("H",0) >= 2: return ["background-color:#1a1500;color:#a07800"]*len(row)
            return [""]*len(row)
